...
class demoSession extends AbstractOriginImplementation
{
    ...
    public function parseData(): int
    {
        ...
        for ($x = 1; $x <= 14; $x++) {
            ...
            $rand = rand();
            $sessionDTO = $this->factory()->session($x)->setParentId('1')->setParentIdType(ISessionDTO::PARENT_ID_TYPE_EXTERNAL_EXT_ID)
                               ...
                               ->setEnd(time() + $x * 600 + 3600)->addTaxonomy($this->taxonomy()->select(
                    'Taxonomy 1'
                )->attach($this->taxonomy()
                               ->node(
                                                                                                                                          'Node Title 1.1'
                                                                                                                                      ))->attach($this->taxonomy()->node(
                    'Node Title 1.2'
                )))->addTaxonomy($this->taxonomy()->select(
                    'Taxonomy 2'
                )
                                      ->attach($this->taxonomy()->node(
                                                                                                                                                                                                                                          'Node Title 2.1'
                                                                                                                                                                                                                                      )))->addMetadata($this->metadata()->getDTOWithIliasId(
                    '1'
                )
                                                                                                                                                                                                                                                            ->setValue(
                                                                                                                                                                                                                                                                                                                 'Meine Metadaten'
