...
final class Logs
{
     ...
     * @var self
     */
    protected static ?Logs $instance = null;
     ...
     * @var stdClass
     */
    protected stdClass $global_additional_data;
    /**
     * @var ILog[][][]
     */
    protected array $kept_logs = [];
    ...
    private function getLogsQuery(
        ...
        if ($limit_start !== null && $limit_end !== null) {
            $sql .= ' LIMIT ' . self::dic()->database()->quote(
                $limit_start,
                "integer"
            ) . ',' . self::dic()->database()->quote($limit_end, "integer");
        }
    ...
    public function getLogById(int $log_id) /*: ?Log*/
    {
        ...
        return Log::where(["log_id" => $log_id])->first();
    }
