...
class SessionOriginConfigFormGUI extends OriginConfigFormGUI {
	...
	protected function addPropertiesDelete() {
		...
		$delete = new ilRadioGroupInputGUI(self::plugin()->translate('sess_prop_delete_mode'), $this->prop(SessionProperties::DELETE_MODE));
		$delete->setValue($this->origin->properties()->get(SessionProperties::DELETE_MODE));

		$opt = new ilRadioOption(self::plugin()->translate('sess_prop_delete_mode_none'), SessionProperties::DELETE_MODE_NONE);
		$delete->addOption($opt);

		$opt = new ilRadioOption(self::plugin()->translate('sess_prop_delete_mode_delete'), SessionProperties::DELETE_MODE_DELETE);
		$delete->addOption($opt);

		$opt = new ilRadioOption(self::plugin()->translate('sess_prop_delete_mode_trash'), SessionProperties::DELETE_MODE_MOVE_TO_TRASH);
		$delete->addOption($opt);
